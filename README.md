...
1. **Memory-mapped I/O** - `mmap()` for zero-copy file reading
...
4. **Fast hash function** - 64-bit multiply-xorshift hash with prefix extraction
5. **Fixed-point arithmetic** - Avoid floating point for temperature calculations
6. **Prefix-first comparison** - Compare 8-byte prefix before full memcmp
...
9. **AVX2 SIMD newline search** - Custom 32-byte-at-a-time scanner
10. **Branch prediction hints** - `__builtin_expect` for temperature parsing
...
Time for 16 threads @ 3.9 GHz (`AMD Ryzen 7 3800XT 8-Core Processor`): **2.46 seconds**
